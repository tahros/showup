...
for(const m of html.matchAll(/src="(js\/[^?"]+)\?v=/g)){
  ...
  if(m[1]==='js/progression.js'&&process.env.PG_MUTATE==='paging')source=source.replace('state.anchor=allDates[nextStart];state.span=start-nextStart','state.anchor=allDates[start];state.span=start-nextStart');
  if(m[1]==='js/progression.js'&&process.env.PG_MUTATE==='linked-anchor')source=source.replace("if(action==='numbers'||action==='dots'){state.mode=action;state.span=action==='dots'?12:4;state.pick=null;}","if(action==='numbers'||action==='dots'){state.mode=action;state.anchor=null;state.span=action==='dots'?12:4;state.pick=null;}");
  if(m[1]==='js/progression.js'&&process.env.PG_MUTATE==='persist-view')source=source.replace('localStorage.setItem(PG_VIEW_KEY,JSON.stringify(progressionViews))','void 0');
  if(m[1]==='js/progression.js'&&process.env.PG_MUTATE==='latest-view')source=source.replace("if(action==='latest'){state.anchor=null;state.span=null;state.pick=null;}","if(action==='latest'){state.span=null;state.pick=null;}");
  ...
  if(m[1]==='js/progression.js'&&process.env.PG_MUTATE==='date-collision')source=source.replace("t.left-ticks[i-1].right>=6","true");
...
assert.match(css,/\.pg-history-row\{height:40px;display:flex;align-items:center;justify-content:space-between/,'Latest shares one stable metadata row');
assert.match(css,/\.pg-latest\{[^}]*min-width:72px[^}]*height:36px[^}]*border-radius:18px/,'Latest is compact but visibly tappable');
...
check('historical ranges offer a contextual Latest action',`!document.querySelector('.pg-latest').hidden&&document.querySelector('.pg-latest').textContent.includes('Latest')`);
...
check('12 to 4 Sessions keeps the same earliest visible session',`document.querySelectorAll('.pg-detail .pg-date').length===4&&document.querySelector('.progression-card')._pg.dates[0]==='2025-12-01'&&progressionUI.train.anchor==='2025-12-01'`);
run(`document.querySelector('[data-pg-action="next-range"]').click();window.pgFourAnchor=document.querySelector('.progression-card')._pg.dates[0];document.querySelector('[data-pg-action="dots"]').click();`);
check('moving 4 Sessions also moves 12 Sessions from the same early anchor',`pgFourAnchor==='2026-06-04'&&document.querySelector('.progression-card')._pg.dates[0]===pgFourAnchor&&document.querySelector('.progression-card')._pg.dates.length===12`);
run(`(()=>{const r=document.querySelector('.pg-scrubber');r.value='3';r.dispatchEvent(new Event('input',{bubbles:true}));window.pgSavedPick=progressionUI.train.pick;})()`);
check('the shared anchor, mode, range and selected set are saved locally',`(()=>{const v=JSON.parse(localStorage.getItem(PG_VIEW_KEY))[progressionViewId('Incline Barbell Bench Press','load')];return v.anchor===pgFourAnchor&&v.mode==='dots'&&v.span===12&&v.pick===pgSavedPick;})()`);
run(`progressionUI.train={mode:'numbers'};document.getElementById('view').innerHTML=progressionSection('Incline Barbell Bench Press','train');bindProgression();`);
check('leaving and reopening the screen restores the same place',`progressionUI.train.mode==='dots'&&document.querySelector('.progression-card')._pg.dates[0]===pgFourAnchor&&progressionUI.train.pick===pgSavedPick&&document.querySelector('.pg-scrubber').value==='3'`);
run(`document.getElementById('view').insertAdjacentHTML('beforeend',progressionSection('Incline Barbell Bench Press','stats'));bindProgression();`);
check('Train and Stats progression charts share the exercise position',`document.querySelector('[data-pg-role="stats"] .progression-card')._pg.dates[0]===pgFourAnchor&&progressionUI.stats.mode==='dots'`);
run(`document.querySelector('[data-pg-role="train"] [data-pg-action="next-range"]').click();`);
check('visible duplicates follow range changes immediately',`document.querySelector('[data-pg-role="train"] .progression-card')._pg.dates[0]==='2026-06-16'&&document.querySelector('[data-pg-role="stats"] .progression-card')._pg.dates[0]==='2026-06-16'`);
run(`document.querySelector('[data-pg-role="stats"]').remove();document.querySelector('.pg-latest').click();`);
check('Latest returns to the newest 12 sessions and then follows newest 4',`document.querySelector('.progression-card')._pg.dates[0]==='2026-08-11'&&document.querySelector('.pg-latest').hidden&&(()=>{document.querySelector('[data-pg-action="numbers"]').click();return document.querySelector('.progression-card')._pg.dates[0]==='2026-08-19'&&document.querySelector('.progression-card')._pg.dates.at(-1)==='2026-08-22';})()`);
check('all numeric pages retain one height, scale and four-column spacing',`(()=>{const c=document.querySelector('.progression-card'),signature=()=>JSON.stringify([c._pg.layout.height,c._pg.layout.col,c._pg.layout.ticks]),frame=signature(),dates=[];do{dates.push(...c._pg.dates);if(signature()!==frame||c.querySelectorAll('.pg-set text').length!==c._pg.scope.length)return false;if(c.querySelector('[data-pg-action="prev-range"]').disabled)break;c.querySelector('[data-pg-action="prev-range"]').click();}while(true);c.querySelector('.pg-latest').click();return dates.length===53&&new Set(dates).size===53&&signature()===frame;})()`);
...
check('short 12-session histories thin dates before labels collide',`(()=>{const dates=['2026-08-11','2026-08-18','2026-08-22','2026-08-29','2026-09-03'],records=dates.flatMap((d,j)=>[135,155,165].map((load,i)=>({d,id:j+':'+i,load,count:8,kind:'load'}))),m=progressionLayout(records,'load',{dates,width:329,dots:true,columns:12,frame:progressionFrame(records,'load',329,4)});return m.col===(329-44)/12&&m.points.length===15&&m.dateTicks.length===3&&m.dateTicks.map(t=>t.label).join(',')==='8/11,8/22,9/3'&&m.dateTicks.every((t,i)=>!i||t.left-m.dateTicks[i-1].right>=6);})()`);
