...
## Setup in a fresh sandbox
    ...
    for t in todayhero settings sessfmt histpart reseal scrollpos exitpair \
             ...
             bwcard sharecard rest import beta theme comeback milestone scrub cards pmix; do
...
- `buildcheck.py DIR` — structural gates: one version everywhere, stamped
  ...
  * no `iBtn` tip over 120 chars, and none passed as a variable (v3.3.71)
  * zoom doctrine: viewport `user-scalable=no` + `html` manipulation + `.zoom` none (v3.3.78)
