...
for(const kind of ['sunday','today','closed','gap','empty','single']){await setup(kind);const summary=await p.locator('.pw-home details>summary').allTextContents();const expected=kind==='today'?7:kind==='gap'?5:kind==='empty'?0:kind==='single'?1:6;assert.equal(summary.length,expected,kind);if(expected){assert(!(await p.locator('.pw-home').textContent()).includes('Plan your next workout'));assert(summary[0].includes(kind==='today'?'Today':kind==='gap'?'Tue, Sep 22':'Tomorrow'));}else assert((await p.locator('.pw-home').textContent()).includes('Plan your next workout'));assert.equal(await p.locator('.pw-home-tools button').count(),2);
  /* v4.6.94: tomorrow never folds away, and the fold opens by default so no
     planned day vanishes from a screen that showed it yesterday. */
  const fold=p.locator('.pw-later-fold');const hasFold=await fold.count();
  if(hasFold){assert(await fold.evaluate(f=>f.open),kind+' fold open by default');
    assert.equal(await p.locator('.pw-later-list>.pw-later-day').count(),kind==='today'?1:0,kind+' rows outside the fold');}
...
{const fold=p.locator('.pw-later-fold');const tall=await fold.evaluate(f=>f.getBoundingClientRect().height);
 await p.locator('.pw-later-fold>summary').click();await p.waitForTimeout(450);
 ...
 assert.equal(await p.locator('.pw-later-fold').evaluate(f=>f.open),false,'still shut after a re-render');
 assert.equal(await p.locator('.pw-later-list>.pw-later-day:visible').count(),0,'tomorrow was the card here, so nothing is left outside');
 await p.locator('.pw-later-fold>summary').click();await p.waitForTimeout(450);
 assert.equal(await p.locator('.pw-later-fold').evaluate(f=>f.open),true,'and it opens again');}
