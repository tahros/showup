...
await_(async()=>{
  ...
  ok("Use for today saves it for today, through planSave", run(`!!planNow() && planNow().items.length===3 && DB.plan.d===todayISO`));
  ...
  run(`document.querySelector('[data-planaccept]').click()`);
  ok("...Use for this week saves the week on the week scope", run(`!!weekNow() && Object.keys(weekNow().days).length===${picked} && lift.planScope==='week'`));
