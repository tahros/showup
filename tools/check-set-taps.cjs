...
(async()=>{const b=await chromium.launch({executablePath:'C:/Users/sungj/AppData/Local/ms-playwright/chromium-1217/chrome-win64/chrome.exe'});try{
 ...
 for(const kind of ['history','plan','now']){
  ...
  const before=await p.evaluate(()=>{window.__ticks=0;repTick=()=>window.__ticks++;repTickInit=()=>{};return JSON.stringify(DB.days[todayISO].w);});
  ...
  assert.equal(await p.evaluate(()=>window.__ticks),1,'One click for changed reps');
  await p.evaluate(()=>plRepFeedback(lift.rep));assert.equal(await p.evaluate(()=>window.__ticks),1,'Same reps stay quiet');
 }
