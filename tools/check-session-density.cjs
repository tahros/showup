...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Users/sungj/AppData/Local/ms-playwright/chromium-1217/chrome-win64/chrome.exe'});try{
 ...
 if(mode!=='high-reps'){assert.equal(await card.locator('.sc-history .sc-rep').count(),record.count);assert.equal(measurement.rows,mode==='screenshot'?4:7);if(mode==='screenshot')assert(measurement.height<430);const before=await p.evaluate(()=>JSON.stringify(DB));await card.locator('[data-sc-details]').click();assert.equal(await card.locator('tbody tr').count(),record.count);await card.locator('[data-sc-details]').click();assert.equal(await p.evaluate(()=>JSON.stringify(DB)),before);}
