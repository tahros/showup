...
Rebuilt 2026-07-21 (container reset lost the original; behavior per handoff doc):
  ...
  - every stamped asset appears in sw.js SHELL (14 assets; progression.js included)
...
RUNTIME = {"--i", "--len", "--sat", "--planw", "--planr", "--h", "--j", "--f"}   # set via style.setProperty / inline style / env() default
...
used = set(re.findall(r"var\((--[A-Za-z0-9-]+)", css+(d/"css/planner.css").read_text()+(d/"css/mascot.css").read_text()))
...
if (d/".git").exists() and _sp.run(["git","-C",str(d),"check-ignore","-q","assets/ios/AppIcon-1024.png"]).returncode == 0:
