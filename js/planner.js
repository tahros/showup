...
function pwTodayHTML(){
  ...
  if(closed&&upcoming){const key='future:'+next;html+=`<details class="pw-saved" data-pw-fold="${key}" ${pwFoldOpen(key)?'open':''}><summary><span>${next===tomorrowISO()?'Tomorrow':hesc(pwDate(next))}</span><span>${upcoming.items.length} exercises</span></summary>${planCardHTML(upcoming,false)}<div class="pw-actions">${pwAction('open-date','Edit','edit','pw-text',`data-date="${next}"`)}${pwAction('paste-open','Paste','paste','pw-text',`data-date="${next}"`)}</div></details>`;}
