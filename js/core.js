...
/* v3.3.162: a snapshot is a photograph of the day BEFORE something was taken
...
function checkDate(){
  ...
  if(now!==todayISO){
    ...
    /* v3.3.162: yesterday just sealed — stats must ingest it, and the rest
...
/* v3.3.162: iOS PWAs resume without reliably firing visibilitychange, and
...
async function load(){
  ...
  if(!DB.days) DB.days={};
  migrateExNames();           // v3.3.162: before anything derives or renders
  _setSig=settingsSig();      // baseline: loading is not an edit
  applyTheme();
}
/* v3.3.162: five catalog names moved to their globally standard forms, and
