...
function restStats(){
  ...
  const first=all[0];
  const days=[]; let restDays=[]; let run=0, longest=0, longestEnd=null; const runBefore={};
  for(let d=new Date(first+'T00:00'); ; d.setDate(d.getDate()+1)){
    const iso=d.toLocaleDateString('en-CA'); if(iso>todayISO) break;
    days.push(iso);
    if(trained.has(iso)){ run++; if(run>longest){ longest=run; longestEnd=iso; } }
  ...
  for(const r of restDays){ const y=new Date(r+'T00:00'); y.setDate(y.getDate()-1); const iso=y.toLocaleDateString('en-CA');
    const sets=byDay[iso]||[]; const parts=new Set(sets.map(x=>x.part).filter(p=>p&&p!=='Run'));
    afterOf[r]=[...parts];
    for(const p of parts) after[p]=(after[p]||0)+1; }
  const afterTop=Object.entries(after).sort((a,b)=>b[1]-a[1])[0]||null;
...
/* v3.3.527: PASTE. The maker's own clipboard glyph read as "a document" --
   ...
   the board is open at the top and an arrow drops into it.
   Drawn to the set's grammar rather than imported: a 100-unit box, stroke 9,
   round caps and joins, same as sparkle and chevron. A traced Noun outline at
   its native weight would have sat noticeably darker than the sparkle three
   ...
   edge and rendered as a notch with the line doubled behind it. The gap and
   the clip are the same span now, and the whole thing is a single stroke: up
