...
function pfDateFooter(){
 ...
 let main=s.busy?action('pf-generate','Creating your draft','sparkle',true):action('pf-generate',fresh.length?'Plan '+plural(fresh.length,'new day'):'Plan new days','sparkle',true,fresh.length?'data-subset="fresh"':'disabled')+action('pf-edit-selected',saved.length?'Edit '+plural(saved.length,'plan'):'Edit saved plans','edit',false,saved.length?'':'disabled');
...
function pfCalendar(){const s=pw(),j=pfState(),base=new Date((s.month||todayISO.slice(0,7)+'-01')+'T12:00'),first=weekStartDow(),offset=(base.getDay()-first+7)%7,n=new Date(base.getFullYear(),base.getMonth()+1,0).getDate();return `<div class="card pf-date-sheet"><div class="pw-month">${pwButton('month',icon('chevron',ICON_SZ.sm,180),'pw-icon','data-delta="-1" aria-label="Previous month"')}<strong>${base.toLocaleDateString('en-US',{month:'long',year:'numeric'})}</strong>${pwButton('month',icon('chevron',ICON_SZ.sm),'pw-icon','data-delta="1" aria-label="Next month"')}</div><div class="pw-calendar pf-calendar">${Array.from({length:7},(_,i)=>`<span>${['S','M','T','W','T','F','S'][(first+i)%7]}</span>`).join('')}${'<span></span>'.repeat(offset)}${Array.from({length:n},(_,i)=>{const date=new Date(base);date.setDate(i+1);const d=pwISO(date),selected=s.dates.includes(d),draft=pfCalendarDraft(d),saved=d>=todayISO&&!!pwSaved(d)?.items?.length;return pwButton('date',`<span>${i+1}</span><small class="pf-day-status" aria-hidden="true">${d<todayISO?'':draft?pfStatusIcon('draft'):saved?pfStatusIcon('saved'):''}</small>`,`${selected?'selected':''} ${draft?'pf-has-draft':''}`,`data-date="${d}" aria-pressed="${selected}" aria-label="${hesc(pwDate(d,true))}, ${d<todayISO?'past date':draft?(saved?'draft, unsaved changes to saved plan':'new unsaved draft'):saved?'plan saved':'no plan'}" ${d<todayISO?'disabled':''}`);}).join('')}</div><div class="pf-calendar-key"><span class="pf-keys"><span><i class="pf-key-selected" aria-hidden="true"></i> Selected</span><span>${pfStatusIcon('saved')} Saved plan</span><span>${pfStatusIcon('draft')} Draft</span></span><span>Up to 7 days</span></div></div>`;}
