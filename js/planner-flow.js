...
function pfCalendar(){const s=pw(),j=pfState(),base=new Date((s.month||todayISO.slice(0,7)+'-01')+'T12:00'),first=weekStartDow(),offset=(base.getDay()-first+7)%7,n=new Date(base.getFullYear(),base.getMonth()+1,0).getDate();return `<div class="card"><div class="pw-month">${pwButton('month',icon('chevron',ICON_SZ.sm,180),'pw-icon','data-delta="-1" aria-label="Previous month"')}<strong>${base.toLocaleDateString('en-US',{month:'long',year:'numeric'})}</strong>${pwButton('month',icon('chevron',ICON_SZ.sm),'pw-icon','data-delta="1" aria-label="Next month"')}</div><div class="pw-calendar pf-calendar">${Array.from({length:7},(_,i)=>`<span>${['S','M','T','W','T','F','S'][(first+i)%7]}</span>`).join('')}${'<span></span>'.repeat(offset)}${Array.from({length:n},(_,i)=>{const date=new Date(base);date.setDate(i+1);const d=pwISO(date),selected=s.dates.includes(d),editing=j.anchor.includes(d),saved=d>=todayISO&&!!pwSaved(d)?.items?.length;return pwButton('date',`<span>${i+1}</span><small class="${saved?'pf-plan-dot':''}" aria-hidden="true">${d<todayISO?'':saved?'●':'○'}</small>`,`${selected?'selected':''} ${editing?'pf-editing':''}`,`data-date="${d}" aria-pressed="${selected}" aria-label="${hesc(pwDate(d,true))}, ${d<todayISO?'past date':saved?'plan saved':'no plan'}${editing?', editing draft':''}" ${d<todayISO?'disabled':''}`);}).join('')}</div><div class="pf-calendar-key"><span><i class="pf-plan-dot">●</i> Plan saved</span><span>○ No plan</span><span class="pf-key-selected">Selected</span><span class="pf-key-editing">Editing</span></div><p class="pw-small">${s.dates.length} selected · Up to 7 days</p></div>`;}
