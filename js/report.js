...
/* v3.3.506: the photo card is removed. It shipped over v3.3.502-504 -- a
   camera on the ceremony, a two-zone composite with the session drawn on its
   own panel below the picture -- and the maker judged it not ready. It goes
   whole rather than sitting unreachable: an entry point nobody can reach is
   still bytes on every device and still breaks against every later change to
   the code around it.
   Restore point: 8bbe7b0 (v3.3.504). drawPhotoCard, _photoCard,
   loadPickedImage, celebrateDayDone's photoSession/photoCard/picker wiring and
   tools/test-photocard.js are all whole and green there. */
