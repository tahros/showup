...
- **House style:** the policy no longer uses "never" ("does not", "no" instead). "Nothing leaves the device" without an account became "Your workouts stay on the device", which is also more exact now that usage counts exist.
- **Logged days, spelled out:** the policy defines a logged day (at least one set or activity recorded; opening the app does not count), and the Settings row reads "Free logged days: N of 20".
- **support.html:** sync instructions name Continue with Apple as well, and say to use the same one on each device.
- **Owner dashboard door:** centred in Settings under the version, a card-button with the mascot beneath the words (owner only).
- **The gym film on a finished day:** a finished Today card is itself the replay button, so the mascot's first tap opened the summary and the secret could not be reached. A tap on the mascot there now waits 450ms (reset by each tap): three taps open the film, one or two open the summary once; taps elsewhere on the card are immediate. `tools/check-secret-closed-day.cjs` (Chromium; fails on v4.6.145).
- `check-gym-tour.cjs` settles the one-time founding-member mark (v4.6.144) before it snapshots the record; v4.6.144 made its "record untouched" comparison fail (a real setting write, not the film). Found running every browser check for this release.
- Runbook: the App Store description, What's New and the App Privacy answers (5.7) were revised to match, in the tracker doc.

## v4.6.145 (2026-09-26) — Sign in with Apple, ready and switched off

- **Built, not visible** (runbook 4.4; App Review 4.8). A native ShowUpApple plugin, generated by `tools/ios-config.py` like Health, Awake and Chrome, shows Apple's own sheet and returns an identity token; `signInApple()` in js/core.js gives it to Supabase (`grant_type=id_token`) with the raw nonce while Apple got its SHA-256. A hide-my-email relay address is an ordinary email.
- **Off until the paid account.** `ios-flags.json` `"appleSignIn": false`: the capability (entitlement) is only added, and the button only shown, when it is true. A personal team cannot sign an app that has the capability, so turning it on before enrollment would break the Mac build.
- **Continue with Apple** sits above Continue with Google in Settings: black on light, white on dark, the system Apple glyph. The note under them now says "private to your account".
- **Revocation** (Apple requires it on account deletion): after sign-in the app sends Apple's one-time code to a new `apple-link` Edge Function, which exchanges it (ES256 client secret, `_shared/apple.ts`) and keeps the refresh token in `apple_tokens` (RLS, no policies). `delete-account` revokes it at Apple before deleting. Both skip Apple quietly until the repo has APPLE_TEAM_ID / APPLE_KEY_ID / APPLE_PRIVATE_KEY.
- **Needs by hand:** the v4.6.145 section of `supabase-setup.sql`; and, after enrollment, the Apple provider in Supabase with the bundle id as its client id, the Sign in with Apple key's secrets in GitHub, and the flag flipped (one Mac rebuild).
- `tools/test-apple-signin.js` (25 checks, client and server, the JWT verified against a real key); `test-ios-config.py` checks the flag both ways; `test-metrics-sql.js` checks the table's security.
...
- A new local plugin, ShowUpChrome (generated by `tools/ios-config.py`, like Health and Awake), paints the web view, its scroll view, the area under the page and the controller's view in the theme's `--ground`. `applyTheme()` sends it on every theme change (`groundNative` in js/util.js), once per colour. The launch colour is now the dark ground, the app's default theme.
- **Needs a Mac rebuild** (native code). Older builds keep taking web updates and simply keep the old band.
- `tools/test-ground.js`; `test-ios-config.py` covers the generated plugin. Checked in Chromium that `--ground` matches the page for dark, light, retro dark and retro light.
