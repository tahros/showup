...
- The iOS app now holds the screen on natively (`UIApplication.isIdleTimerDisabled`) through a small ShowUpAwake plugin, generated by `tools/ios-config.py` alongside the Health plugin and registered by ShowUpViewController.
- **The rule is the maker's option B:** it stays on while the workout is live, the phone is sideways and the rest timer is showing, and lets go the moment any of those stops. That includes 30 minutes after your last set, when the timer itself ends, and whenever the app is hidden. Nothing keeps a phone lit in a pocket or on a bench.
- The web app is unchanged (Safari honours the Wake Lock). An iOS build without the plugin falls back to what it had.
- **Needs a Mac rebuild** (`git pull && npm run sync:ios`, then ▶).
- `tools/test-awake.js` covers upright, sideways, told once, the 30-minute end, finishing, hidden, the header-only bridge, an older build and the website. The generated-project tests check the plugin and its registration.
...
km while every other run figure on the page read mi. One unit per app: the
caption is a label, the stored preference is ignored, and the card follows
lb → mi, kg → km like the Running month card above it.

**The pace axis calibrates to the body of the runs, not the outliers.** One
walked run in 2021 at 41'/km stretched the axis to 41' and flattened five years
of 6'30" runs into a line at the bottom. The range is now the 5th–95th
percentile of the paces, padded; anything outside is drawn at the rim, hollow,
so it is still there but does not own the axis. Steps are the ones a runner
...
- Bumped every shell asset URL and the service-worker cache so affected devices cannot reuse the damaged v3.3.217 files.

## v3.3.217 (2026-08-12) — Stats, made actionable

- Growth Audit now puts each exercise's load × reps record directly in the row and calls out a recent record change.
...
consistent but not conceptually comparable — a Smith number, dumbbell pair,
