...
1. WEEK FIRST. Read payload.week_context before choosing the requested day. It covers the requested date through Sunday. A future entry with planned is already saved and is a fixed constraint: do not rewrite it, duplicate its main work on the requested day, or consume the recovery it needs. For unplanned usual training days, make a quiet provisional outline through Sunday so today's choice leaves a coherent remainder. Do not output that outline.
...
rest-of-week context through Sunday (requested dates are writable; future planned blocks are fixed): ${JSON.stringify(payload.week_context || [])}
